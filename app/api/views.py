...
def create_attendant():
    ...
    if role != "admin":
        return json_mesages("Access denied","Ask your administrator the right to access these resources"), 403
    try:
...
def login():
    """
    This endpoint is used by the user to login
    """
    try:
        json.loads(request.get_data())
    except (ValueError, TypeError):
    ...
    return json_er("User with name {} does not have an account".format(data['username'])), 401
...
def update_user_role(id):
    ...
    if is_valid_role(data["user_role"]) != "Valid":
        return json_er(is_valid_role(data["user_role"])), 417
...
def create_product():
    ...
    role = get_jwt_identity()["user_role"]
    if role != "admin":
        return json_mesages("Access denied", "You have no right to access this resource"), 403
    try:
        json.loads(request.get_data())
        data = request.get_json(force=True)
    ...
    if not "product_name" in data or not "price" in data or not "quantity" in data or not "min_qty_allowed" in data:
        return json_er("Please make sure name, price, quantity, quantity_allowed are in the request"), 400
...
def delete_product(id):
    ...
    return json_msg(delete, "Product", deleted_product), 202
...
def create_sale_order():
    ...
    role = get_jwt_identity()["user_role"]
    if role != "attendant":
        return json_mesages("Access denied","You have no rights to access these resources"), 403
    try:
        json.loads(request.get_data())
    except (ValueError, TypeError):
        return json_er("Bad request, your request should be a dictionary"), 400
    ...
    if not "product_id" in data or not "quantity" in data or not "attendant_name" in data: 
        return json_er("Please make sure product_id, quantity and attendant_name are included int he request"), 400
